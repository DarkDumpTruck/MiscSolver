...
struct GuessPlane
{
    ...
    int dfs(int x, int start, const board &bodys, const board &heads, const board &info_head,
            const board &info_body, const board &info_empty, vector<int> &p_head,
            vector<int> &p_body, vector<int> &p_empty)
    {
        if (x == PLANE)
        {
            if ((info_body & bodys) != info_body)
                return 0;
            if ((info_head & heads) != info_head)
                return 0;
            if ((info_body & heads).any())
                return 0;
            if ((info_empty & bodys).any())
                return 0;
            if ((info_empty & heads).any())
                return 0;
             ...
             const board &info_body, const board &info_empty)
    {
        ...
        for (int i = start; i < N * N;
             i++)
        { // TODO: maybe give weight to each plane by its head position
            for (int di = 0; di < 4; di++)
            {
                if (valid[i][di])
                {
                                                       ...
                                                       double beta = 0)
    {
        ...
        int count = dfs(0, 0, {}, {}, info_head, info_body, info_empty, p_head, p_body,
                        p_empty);
    ...
    double expected_times(const board &current_info_head,
                          ...
                          const board &target_hd, int depth, double alpha, double beta)
    {
        auto mixed_actions =
            calculate(current_info_head, current_info_body, current_info_empty, alpha, depth ? beta : 0);
        double result = 0;
        ...
        if (target_hd.test(a))
        {
            ...
            else
            {
                ...
                result +=
                    expected_times(info_head, current_info_body, current_info_empty,
        ...
        else
        {
            ...
            if (depth == 0)
            {
                result += 1 + expected_times(current_info_head, current_info_body,
                                             info_empty, target_bd, target_hd, 2, alpha, beta);
            }
            else
            {
    ...
    void benchmark(double alpha, double beta)
    {
        ...
        dfs2(0, 0, {}, {}, bodys_list, heads_list, info_head, info_body,
            info_empty);
